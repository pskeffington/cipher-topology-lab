...
External randomness testing is supported but may be unavailable on a local machine if `dieharder` is not installed. The external-randomness runner writes an explicit status report even when Dieharder is missing, so evidence files distinguish unavailable external infrastructure from completed external analysis. The current local weak-control evidence register records external testing as unavailable, not completed.
...
`v0.4.1-pre.0`: execution-repair pre-release with deterministic baseline, corrected AES-CTR metadata language, stratified TDA outputs, standardized distance metrics, object-oriented segmented workflow, 64-replicate `ripser` evidence, weak-control sensitivity extension, related-work manuscript section, failure-safe external-randomness status reporting, and dated documentation updates for the artifact/figure inventory and weak-control evidence boundary.
